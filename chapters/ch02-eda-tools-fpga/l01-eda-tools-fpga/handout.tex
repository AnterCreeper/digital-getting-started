...
在第一章里，我们站在远处看了一座城的轮廓——芯片是什么，系统怎么组织，产业链如何分工。上一章最后我们提到：当芯片制造本身已经变成可重复、可组织、可协作的工业流程时，设计环节也必然发展出对应的工具链与工程方法。从本章开始，你要走进这座城，第一次真正坐在一台Linux机器前，安装工具、运行脚本、观察输出，并借助一个最小的实验获得可验证的正反馈。

很多初学者会问：为什么不先学电路原理，再碰工具？答案很简单——\textbf{即时反馈是认知杠杆}。当你写下几行RTL代码，几秒钟后在波形窗口里看到信号随你的代码变化，这种直接关联比硬背两章布尔代数再碰工具要有效得多。本章的目标不是让你成为工具专家，而是让命令行工作方式和EDA工具链闭环变成你的``身体经验''。

这一章也不是Linux系统管理入门课，不是Verilog语法教材（那是第四章的事），更不是深入讲解EDA原理的专题。它的教学契约很明确：\textbf{先会跑通，再会解释；先会观察波形，再会设计电路；先建立工具链直觉，再追究每个参数的含义}。

全章围绕一个实验贯穿始终：呼吸灯PWM计数器（lab1-breathing-led）。从安装工具、跑仿真、看波形，到综合出门级网表，全用这一个例子。认知负荷保持恒定，工具概念一层层叠加上去。
...
本章把这个顺序颠倒过来：\textbf{先让你动手跑通一个最小闭环，再在这个闭环的基础上逐层追加理论}。你不需要先理解PWM计数器的内部实现才能跑仿真——恰恰相反，当你看到波形里的三角波和PWM输出时，你会自然地想知道``这个三角波是怎么生成的''``占空比怎么控制亮度''，这些问题会引导你后续去深入学习。

\subsection{教学闭环的第一次闭合}
...
你的个人文件通常放在 \texttt{/home/<username>} 这个目录下。比如你登录系统的用户名是 \texttt{student}，你的主目录就是 \texttt{/home/student}。系统提供了一个快捷符号 \texttt{\textasciitilde}（波浪号）来代表这个位置，所以 \texttt{cd \textasciitilde} 等价于 \texttt{cd /home/student}。
...
\subsection{绝对路径 vs. 相对路径}

现在来看 \texttt{bash sim/run.sh} 这个命令。当你在 \texttt{lab1/} 目录里执行它时，Shell会先在当前目录下找 \texttt{sim/} 子目录，再在里面找 \texttt{run.sh}——这能成功，因为 \texttt{lab1/sim/run.sh} 确实存在。
...
注意当你从 \texttt{lab1} 进入 \texttt{sim/} 后，提示符变成了 \texttt{\textasciitilde/projects/lab1/sim}；当你用 \texttt{cd ..} 退回上级目录后，提示符又变回了 \texttt{\textasciitilde/projects/lab1}。终端一直在告诉你``你现在在哪''。
\end{warnbox}

\subsection{第一批必须会用的命令}
...
当你第一次进入lab1-breathing-led目录时，会看到这样一个结构：
...
仿真不是``点一个按钮然后看结果''那么简单——它是一条由多个工具组成的流水线。理解这条流水线的意义在于：当某一步出错时，你能判断问题出在编译阶段、运行阶段还是显示阶段，而不是对着黑屏干瞪眼。
...
\texttt{Z} 则表示高阻态，常见于三态总线或暂时无人驱动的引脚。对本章这个实验来说，你更常见的是 \texttt{0}/\texttt{1} 和偶尔需要解释清楚的 \texttt{X}；\texttt{Z} 先知道它是什么即可。
...
\texttt{mode} 是一个控制信号。当它为 \texttt{0} 时，系统选择呼吸灯模式；当它为 \texttt{1} 时，切换到闪烁模式。从截图里可以看到 \texttt{mode} 的变化：在某段时间它是 \texttt{0}，然后跳变为 \texttt{1}，这就解释了为什么后面的波形会发生切换。
...
\texttt{pwm\_out} 是最终送到 LED 的信号。注意它的行为：当 \texttt{mode=0} 时，\texttt{pwm\_out} 完全跟随 \texttt{breathing\_out}；当 \texttt{mode} 变为 \texttt{1} 后，\texttt{pwm\_out} 立刻切换到跟随 \texttt{blink\_out}。这就是一个典型的``二选一''多路选择器在波形上的表现。
...
对这段代码来说，最朴素的理解已经够用了：当 \texttt{condition} 为假时，\texttt{out} 该取什么值并没有被清楚说明。仿真里你可能只是看到 \texttt{out} 保持旧值；综合时，工具为了实现这种“保持”的效果，可能会推断出一个锁存器。问题的重点不是你现在就要掌握 latch 的全部细节，而是要记住：\textbf{仿真看起来“能跑”，并不代表结构上就一定符合你的原意。}
...
想象你有一块巨大的棋盘。棋盘上的每个格子都不是空的，而是预先造好了一个小型计算单元。这些单元之间由可编程的``电线''连接。当你下载一个配置文件到FPGA时，你实际上在做两件事：告诉每个格子``你现在要执行什么逻辑''，以及告诉格子之间的连线``你应该把谁的输出接到谁的输入上''。
...
\begin{itemize}
  \item \textbf{Linux命令行}不是GUI的``高级版''，而是更适合工程重复性和可复现性的工作台
