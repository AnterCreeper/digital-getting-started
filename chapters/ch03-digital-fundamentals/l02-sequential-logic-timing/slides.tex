...
\begin{itemize}
  ...
  \item 能对简单寄存器到寄存器路径计算最基本的 setup / hold 延迟约束
\end{itemize}
\end{frame}
...
\begin{enumerate}
  ...
  \item 先对零 skew 路径做 setup / hold 计算
  \item 再看 skew / uncertainty 怎样修正这些约束
  \item 异步输入与跨时钟域为什么会挑战数字抽象
  \item 状态机只是这种时序组织方式的常见骨架
\end{enumerate}
\end{frame}
...
\begin{columns}[T]
  ...
  \vspace{0.4em}
  \scriptsize 这张主从触发器图按真实结构复刻；本讲重点先放在它抽出的边沿采样语义与时序窗口。
\end{columns}
\end{frame}
...
\begin{frame}{The Minimal Timing Calculation}
\small
先在 \textbf{零 skew、零 uncertainty} 的简化情形下计算：

\vspace{0.5em}
\begin{block}{Setup check}
\centering
\texttt{Tclk >= t\_clk->q,max + t\_comb,max + t\_setup}
\end{block}

\begin{block}{Hold check}
\centering
\texttt{t\_clk->q,min + t\_comb,min >= t\_hold}
\end{block}

\vspace{0.4em}
\begin{itemize}
  \item setup 看的是 \textbf{最长路径} 能否在一个周期内赶到
  \item hold 看的是 \textbf{最短路径} 会不会过早冲到终点寄存器
  \item 后续再把 skew / uncertainty 作为修正项加进去
\end{itemize}
\end{frame}

\begin{frame}{Adding Skew and Uncertainty}
\small
在真实系统里，时钟到达两端寄存器的时刻并不完全相同，因此要把修正项加回去：

\vspace{0.5em}
\begin{block}{Setup intuition}
\centering
\texttt{Tclk >= t\_clk->q,max + t\_comb,max + t\_setup + uncertainty - useful skew}
\end{block}

\begin{block}{Hold intuition}
\centering
\texttt{t\_clk->q,min + t\_comb,min >= t\_hold + uncertainty + harmful skew}
\end{block}

\vspace{0.4em}
\begin{itemize}
  \item 对 setup 有利的 skew 可以“借一点时间”
  \item 对 hold 不利的 skew 会让终点寄存器更晚关窗
  \item 第一次教学先抓方向感，不要求在这一讲把所有符号约定一次背完
\end{itemize}
\end{frame}
...
\begin{frame}{State Machine as a Structured Use of Registers}
\begin{columns}[T]
  ...
  \begin{itemize}
    ...
    \item 这一讲先把它当作“寄存器 + 组合逻辑”的标准骨架，不展开编码和实现细节
  \end{itemize}
\end{columns}
\end{frame}
...
\begin{itemize}
  \item 寄存器把系统从组合语义推进到状态语义，寄存器边界决定了系统如何被切分
  \item setup / hold 先用最长路径 / 最短路径计算，skew / uncertainty 再作为修正项加入
  \item metastability 提醒我们：异步世界总会挑战数字抽象，只能通过同步策略降低风险
  \item 状态机是这套时序组织方式的常见骨架；进入下一章前，先把语义边界看清楚比背语法更重要
\end{itemize}
\end{frame}
...
\begin{enumerate}
  ...
  \item 给定 \texttt{t\_clk->q}、\texttt{t\_comb,max}、\texttt{t\_comb,min}、\texttt{t\_setup}、\texttt{t\_hold}，计算该路径是否满足 setup / hold
