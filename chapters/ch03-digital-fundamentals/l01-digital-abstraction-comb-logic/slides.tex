...
\begin{frame}{MOS Minimal Intuition: Gate-Controlled Switch}
\begin{columns}[T]
  ...
  \vspace{0.3em}
  \scriptsize NMOS：擅长下拉，输入高时更容易导通
  ...
  \vspace{0.3em}
  \scriptsize PMOS：擅长上拉，输入低时更容易导通
\end{columns}

\vspace{0.5em}
\small
单个 MOS 不是理想开关，但它已经给了我们最关键的组织方式：\textbf{用栅极电压去控制电流通路是否存在。}
\end{frame}
...
\begin{columns}[T]
  ...
  \centering
  \includegraphics[width=0.88\linewidth]{fo4.png}

  \vspace{0.3em}
  \scriptsize 更大的负载与 fanout 会拖慢翻转
...
\begin{columns}[T]
  ...
  \centering
  \includegraphics[width=0.95\linewidth]{nand-nor-schema.png}
